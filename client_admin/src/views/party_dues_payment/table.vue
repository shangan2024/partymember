		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/party_dues_payment/table','add') || $check_action('/party_dues_payment/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/party_dues_payment/table','del') || $check_action('/party_dues_payment/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','date')" min-width="200">
						<template slot-scope="scope">
					{{ $toTime(scope.row["date"],"yyyy-MM-dd hh:mm:ss") }}
				</template>
					</el-table-column>
					<el-table-column prop="party_fee_is_required" @sort-change="$sortChange" label="需交党费"
				v-if="user_group == '管理员' || $check_field('get','party_fee_is_required')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/party_dues_payment/table','set') || $check_action('/party_dues_payment/view','set') || $check_action('/party_dues_payment/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/party_dues_payment/table','set') || $check_action('/party_dues_payment/view','set') || $check_action('/party_dues_payment/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/party_dues_payment/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/party_dues_payment/get_list?like=0",
				url_del: "~/api/party_dues_payment/del?",
		...
		methods: {
			...
			get_user_student_id(id){
				...
				return ret;
													...
													openPayModal(v){
				this.pay_obj.id = v.party_dues_payment_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/party_dues_payment/set?party_dues_payment_id="+this.pay_obj.id);
