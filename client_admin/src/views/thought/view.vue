	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="学号" prop="student_id">
							...
							<!--v-if="user_group === '管理员' || (form['thought_id'] && $check_field('set','student_id')) || (!form['thought_id'] && $check_field('add','student_id'))" :disabled="disabledObj['student_id_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','student_id')">{{form['student_id']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['thought_id'] && $check_field('set','student_id')) || (!form['thought_id'] && $check_field('add','student_id'))" id="student_id" v-model="form['student_id']" :disabled="disabledObj['student_id_isDisabled']">
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form['thought_id'] && $check_field('set','full_name')) || (!form['thought_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
				...
				<el-form-item label="标题" prop="title">
							  ...
							  v-if="user_group === '管理员' || (form['thought_id'] && $check_field('set','title')) || (!form['thought_id'] && $check_field('add','title'))" :disabled="disabledObj['title_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','title')">{{form['title']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','thought_report') || $check_field('add','thought_report') || $check_field('set','thought_report')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="思想汇报" prop="thought_report">
												<div v-if="disabledObj['thought_report_isDisabled']">
						<div v-if="$check_field('get','thought_report')">
							<el-button type="primary" @click="$download($fullUrl(form['thought_report']),form['thought_report'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['thought_id'] && $check_field('set','thought_report')) || (!form['thought_id'] && $check_field('add','thought_report'))" class="upload-demo" drag
								   action="" style="max-width: 300px;width: 100%;" :http-request="upload_thought_report" :limit="1" accept="">
							<i class="el-icon-upload"></i>
							<div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
						</el-upload>
						<div v-else-if="$check_field('get','thought_report')">
							<el-button type="primary" @click="$download($fullUrl(form['thought_report']),form['thought_report'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "thought_id",
				url_add: "~/api/thought/add?",
				url_set: "~/api/thought/set?",
				url_get_obj: "~/api/thought/get_obj?",
				url_upload: "~/api/thought/upload?",
				...
				disabledObj:{
								"student_id_isDisabled": false,
										...
										"thought_report_isDisabled": false,
										},
		...
		methods: {
			 ...
			 */
			upload_thought_report(param){
						this.uploadFile(param.file, "thought_report");
					},
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/thought/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/thought/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/thought/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/thought/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/thought/view','get');
