							...
							<div class="form_register">
								<b-form>
									...
									<form_ordinary_users v-if="form.user_group === '普通用户'" :form="form_sub"></form_ordinary_users>
											<!--普通管理者-->
									<form_general_manager v-if="form.user_group === '普通管理者'" :form="form_sub"></form_general_manager>
...
<script>
	...
	import form_ordinary_users from "@/components/diy/form_ordinary_users";
			/* 普通管理者 */
	import form_general_manager from "@/components/diy/form_general_manager";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/ordinary_users/count?", {"student_id": form_sub["student_id"]});
							if(res.result){
								ret = "学号已存在!";
							}
						}
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/general_manager/count?", {"student_id": form_sub["student_id"]});
							if(res.result){
								ret = "学号已存在!";
							}
						}
