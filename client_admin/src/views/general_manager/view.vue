		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="学号" prop="student_id">
							  ...
							  v-if="user_group === '管理员' || (form_sub['general_manager_id'] && $check_field('set','student_id')) || (!form_sub['general_manager_id'] && $check_field('add','student_id'))"></el-input>
				...
				<el-form-item label="管理者姓名" prop="manager_name">
												<el-input id="manager_name" v-model="form_sub['manager_name']" placeholder="请输入管理者姓名"
							  v-if="user_group === '管理员' || (form_sub['general_manager_id'] && $check_field('set','manager_name')) || (!form_sub['general_manager_id'] && $check_field('add','manager_name'))"></el-input>
					<div v-else-if="$check_field('get','manager_name')">{{form_sub['manager_name']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','number_of_sessions') || $check_field('add','number_of_sessions') || $check_field('set','number_of_sessions')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="届数" prop="number_of_sessions">
												<el-input id="number_of_sessions" v-model="form_sub['number_of_sessions']" placeholder="请输入届数"
							  v-if="user_group === '管理员' || (form_sub['general_manager_id'] && $check_field('set','number_of_sessions')) || (!form_sub['general_manager_id'] && $check_field('add','number_of_sessions'))"></el-input>
					<div v-else-if="$check_field('get','number_of_sessions')">{{form_sub['number_of_sessions']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="年级班级" prop="grade_class">
												<el-input id="grade_class" v-model="form_sub['grade_class']" placeholder="请输入年级班级"
							  v-if="user_group === '管理员' || (form_sub['general_manager_id'] && $check_field('set','grade_class')) || (!form_sub['general_manager_id'] && $check_field('add','grade_class'))"></el-input>
					<div v-else-if="$check_field('get','grade_class')">{{form_sub['grade_class']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="院别" prop="hospital">
								<el-select id="hospital" v-model="form_sub['hospital']"
						v-if="user_group === '管理员' || (form_sub['general_manager_id'] && $check_field('set','hospital')) || (!form_sub['general_manager_id'] && $check_field('add','hospital'))">
						<el-option v-for="o in list_hospital" :key="o['department_category']" :label="o['department_category']"
							:value="o['department_category']">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','hospital')">{{form_sub['hospital']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				form_sub: {
								"student_id":  '', // 学号
										"manager_name":  '', // 管理者姓名
										"number_of_sessions":  '', // 届数
										"grade_class":  '', // 年级班级
										"hospital":  '', // 院别
									"examine_state": "未审核",
						    "user_id": 0,
					"general_manager_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["general_manager_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?general_manager_id=" + form_sub["general_manager_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/general_manager/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/general_manager/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/general_manager/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/general_manager/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/general_manager/view','get');
